 ...
 * @since [产品/模块版本]（可选）
 */
@NettyClientHandler(order = 2)
public class ClientHandler2 implements NettyClientMessageHandler {

    private Logger logger = LoggerFactory.getLogger(ClientHandler2.class);
    @Override
    public void handleResponse(NettyResponse response) {
        logger.info("client2接收消息:" + response);
